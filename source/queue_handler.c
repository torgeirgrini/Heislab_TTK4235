...
int queue_get_order(Elevator *p_elev, HardwareOrder order_type, int floor) {
    return p_elev->pp_queue_matrix[order_type][floor];
}
...
void queue_delete_order(Elevator *p_elev, HardwareOrder order_type, int floor) {
    p_elev->pp_queue_matrix[order_type][floor] = 0;
}
...
int queue_active_orders_all_floors(Elevator *p_elev) {
    for(int i = 0; i < ELEVATOR_NUMBER_OF_ORDERS; i++) {
        for(int j = 0; j < HARDWARE_NUMBER_OF_FLOORS; j++) {
            if (p_elev->pp_queue_matrix[i][j]) {return 1;}
        }
    }
    return 0;
}
...
HardwareMovement queue_get_movement_direction(Elevator *p_elev) {
    ...
    case HARDWARE_MOVEMENT_UP:
        ...
        for(int j = start_floor_search; j < end_floor_search; j++) {
            if (p_elev->pp_queue_matrix[HARDWARE_ORDER_UP][j] || p_elev->pp_queue_matrix[HARDWARE_ORDER_INSIDE][j]) {return HARDWARE_MOVEMENT_UP;}
        }
        break;
    ...
    case HARDWARE_MOVEMENT_DOWN:
        ...
        for(int j = start_floor_search; j == end_floor_search; j--) {
            if (p_elev->pp_queue_matrix[HARDWARE_ORDER_DOWN][j] || p_elev->pp_queue_matrix[HARDWARE_ORDER_INSIDE][j]) {return HARDWARE_MOVEMENT_DOWN;}
        }
        break;
    }
